...
# =============================================================
Sys.setenv(RETICULATE_PYTHON = "/Users/muhamadsyukron/opt/anaconda3/envs/lgbm-env/bin/python")
...
tryCatch({
  use_condaenv(
    condaenv = "lgbm-env", 
    conda = "/Users/muhamadsyukron/opt/anaconda3/bin/conda",
    required = TRUE
  )
  cat("--- Berhasil terhubung ke Conda environment 'lgbm-env' ---\n")
}, error = function(e) {
  stop("Gagal terhubung ke Conda. Pastikan path ke conda dan environment sudah benar.")
})
...
for (scenario in simulation_scenarios) {
  ...
  for (i in 1:N_SIMULATIONS) {
    ...
    gc()  # 🧹 clear memory before each run
    ...
    true_simulation <- simulate_thomas_process(
      covariate_names = sim_cov_names, coefficients = sim_coefficients,
      bci_covars = bci.covars, bei_window = win, n_points = 100, 
      scale_factor = scale_factor, intensity_form = intensity_form
    )
