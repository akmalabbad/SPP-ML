# =============================================================
# BAGIAN 1: DEFINISI FUNGSI-FUNGSI OBJECTIVE & METRIC UNTUK LIGHTGBM
# =============================================================

# --- Objective & Metric untuk 'poisson' dan 'weighted_poisson' ---
# Diambil dari skrip asli Anda
make_poisson_obj_lgb <- function(vol_vec) {
  function(preds, dtrain) {
    labels <- get_field(dtrain, "label"); weights <- get_field(dtrain, "weight")
...
make_weighted_poisson_obj_lgb <- function(vol_vec, F_prime_val, hess_min = 1e-6) {
...
make_poisson_metric_lgb <- function(vol_vec) {
  .metric <- function(preds, dtrain) {
    labels <- get_field(dtrain, "label"); event <- which(labels > 0); dummy <- which(labels < 0)
    ...
    list(name = "poisson_err", value = sum(err) / 1e6, higher_better = FALSE)
  }
  return(.metric)
}


# --- Objective & Metric untuk 'logistic' (versi LightGBM) ---
# Menggunakan logika yang sama seperti pada 'xgbpp' tetapi dengan sintaks 'get_field'
make_logistic_obj_lgb <- function(vol_vec, F_prime_val, hess_min = 1e-6) {
  force(vol_vec); force(F_prime_val); force(hess_min)
  function(preds, dtrain) {
    label  <- get_field(dtrain, "label") # Perubahan: get_field
    weight <- 1 / (1 + exp(preds) * F_prime_val)
    weight <- weight / sum(weight[label < 0] * vol_vec[label < 0])
    pos <- label > 0; neg <- !pos
    grad <- numeric(length(label)); hess <- numeric(length(label))
...
make_logistic_metric_lgb <- function(vol_vec) {
  .metric <- function(preds, dtrain) {
    labels <- get_field(dtrain, "label") # Perubahan: get_field
    ...
    # Format return LightGBM membutuhkan `higher_better`
    list(name = "logistic_err", value = sum(err) / 1e6, higher_better = FALSE)
  }
  return(.metric)
}
...
# =============================================================
# BAGIAN 2: FUNGSI WRAPPER UTAMA "lgbpp"
# =============================================================

lgbpp <- function(vol, params, ..., loss = "poisson", F_prime = 1) {
  
  # --- 1. Pilih Objective Function ---
  objective_func <- switch(
    loss,
    "poisson"          = make_poisson_obj_lgb(vol),
    "weighted_poisson" = make_weighted_poisson_obj_lgb(vol, F_prime),
    "logistic"         = make_logistic_obj_lgb(vol, F_prime),
  ...
  eval_func <- switch(
    loss,
    "poisson"          = make_poisson_metric_lgb(vol),
    "weighted_poisson" = make_poisson_metric_lgb(vol),
    "logistic"         = make_logistic_metric_lgb(vol),
    stop("Loss '", loss, "' tidak dikenali.")
  )
  
  # --- 3. Penetapan Parameter & Pemanggilan lgb.train ---
